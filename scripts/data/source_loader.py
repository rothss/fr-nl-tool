...
from query_opm_nl import run_fast_future_kzl_export, run_fast_single_margin_export, run_generic_live_export


def _source_from_probe(file_path: str | None, report_name: str | None, probe: dict, refreshed: bool = False, refresh_message: str | None = None) -> dict:
    source_type = "live_refresh" if refreshed else ("local_file" if probe.get("file_exists") else "query_pipeline")
    return {
...
def _attempt_structured_refresh(plan: dict, intent: dict, file_path: str | None, report_name: str | None) -> tuple[bool, str | None, str | None]:
    ...
    if rn == "单机边际贡献":
        ...
        return ok, fp, None if ok else msg
    if rn == "航空集团前十后十航班":
        report_cpt = find_report_cpt_path(rn, fp) or "doc/Fdjt/市场监督/航空集团前十后十航线.cpt"
    ...
    if plan.get("report_path") and os.path.splitext(str(plan.get("report_path") or ""))[1].lower() == ".xlsx":
        report_cpt = find_report_cpt_path(rn, fp)
        if report_cpt:
            ok, msg = run_generic_live_export(report_cpt, fp, intent)
            return ok, fp, None if ok else msg
...
def acquire_source(plan: dict, intent: dict, query_result: dict) -> dict:
    ...
    if query_result.get("used_live_refresh"):
        refresh_message = str(query_result.get("live_refresh_error") or "").strip() or None
        return _source_from_probe(file_path, report_name, probe, refreshed=True, refresh_message=refresh_message)

    if not plan.get("require_live_refresh"):
        return _source_from_probe(file_path, report_name, probe, refreshed=False, refresh_message=None)

    ok_refresh, refreshed_path, refresh_err = _attempt_structured_refresh(plan, intent, file_path, report_name)
    if ok_refresh:
        refreshed_probe = probe_local_file_against_intent(refreshed_path, report_name, intent)
        return _source_from_probe(refreshed_path, report_name, refreshed_probe, refreshed=True, refresh_message=None)

    return _source_from_probe(file_path, report_name, probe, refreshed=False, refresh_message=refresh_err)
