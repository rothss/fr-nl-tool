...
def choose_top_candidate(ranked: list[dict], intent: dict) -> dict:
    ...
    if bool(filters.get("compare_scope") == "airline_yoy") or (
        ...
        for c in ranked:
            ...
            if "集团经营提升分析" in rn:
                return c
        fixed = default_mirror_root() / "航空板块经营报表" / "集团经营提升分析.xlsx"
        return {
            "report_id": -2,
            "report_name": "集团经营提升分析",
            "file_path": str(fixed),
            "dir_path": "航空板块经营报表",
            "score": 0,
            "score_breakdown": {},
    ...
    if (
        ...
        for c in ranked:
            ...
            if "前十后十" in rn:
                return c
        fixed = default_mirror_root() / "航空板块经营报表" / "集团前十后十航班.xlsx"
        if fixed.exists():
            return {
                "report_id": -1,
                "report_name": "集团前十后十航班",
                "file_path": str(fixed),
                "dir_path": "航空板块经营报表",
                "score": 0,
                "score_breakdown": {},
    ...
    if has_route_or_time:
        ...
        fixed = default_mirror_root() / "指定航线" / "未来航班客座率票价分析.xlsx"
        if fixed.exists():
            return {
                "report_id": -1,
...
def run_fast_single_margin_export(output_file: str) -> tuple[bool, str]:
    ...
    args = [
        ...
        str(script),
        "--report-path",
        "reportlets/demo/财务报表/单机边际贡献(变动成本新口径).cpt",
        "--output-file",
        output_file,
    ]
    try:
...
def is_fast_top_metric_flight_query(intent: dict) -> bool:
    ...
    return (
        ...
        and ("集团" in raw_query or str(filters.get("group") or "") == "集团")
    )
...
def lookup_profit_yoy_snapshot(intent: dict) -> dict | None:
    ...
    if not isinstance(worst, dict):
        return None
    return {
        "report_name": str(snap.get("report_name") or "集团经营提升分析"),
...
def run_query(
    ...
    if bool((intent.get("filters") or {}).get("compare_scope") == "airline_yoy"):
        if str(filters.get("report_variant") or "") == "adjusted_profit_overview":
            ...
            top = {
                ...
                "report_name": "集团经营提升分析",
                "file_path": str(
                    mirror_root / "航空板块经营报表" / "集团经营提升分析.xlsx"
                ),
                "dir_path": "航空板块经营报表",
                "score": 0,
                "score_breakdown": {},
    ...
    if (not report_cpt) and ("经营提升分析" in str(top.get("report_name") or "")):
        report_cpt = (
            "reportlets/demo/marketOperSup/集团经营提升分析/集团经营提升分析.frm"
        )
    if (not report_cpt) and (
        "收入利润概览（调整后）" in str(top.get("report_name") or "")
    ):
        report_cpt = "doc/frm/集团收入利润报表/集团收入利润概览（调整后）.frm"
    query_hash = hashlib.md5(
        str(intent.get("raw_query") or "").encode("utf-8")
    ).hexdigest()[:10]
    ...
    if bool(filters.get("compare_scope") == "airline_yoy"):
        ...
        base_name = Path(str(top.get("file_path") or "")).stem or "集团经营提升分析"
        comp_live_file = (
            mirror_root
            / "航空板块经营报表"
