...
from profiles.fr_example import profile_metadata
...
def _write_airline_yoy_report(path: Path, today: date) -> None:
    ...
    rows = [
        [snapshot_date, "CompanyA", "12%", 1],
        [snapshot_date, "CompanyC", "4%", 5],
        [snapshot_date, "CompanyB", "-8%", 11],
    ]
    for row in rows:
        ws.append(row)
    path.parent.mkdir(parents=True, exist_ok=True)
        ...
        "entries": [
            {
                ...
                "reports": [
                    {
                        "name": future_name,
                        "path": "reportlets/demo/demo/future_flight_competition.cpt",
                    }
                ],
            },
            {
                ...
                "reports": [
                    {
                        "name": airline_name,
                        "path": "reportlets/demo/demo/airline_yoy.cpt",
                    }
                ],
            },
        ]
...
def create_demo_mirror(root: Path, today: date | None = None) -> dict:
    ...
    meta = profile_metadata()
    future_report = (
        root / "指定航线" / f"{meta['report_names']['future_flight_competition']}.xlsx"
    )
    airline_report = (
