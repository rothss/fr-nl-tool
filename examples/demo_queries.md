...
```powershell
python scripts/runner.py "海口-北京首都的指定航线，近三天的票价和客座率与外航相比，有没有什么异常或者可以改进的吗" --mirror-root examples/demo_mirror --db examples/demo_mirror/search_index/report_catalog.db --user-scope examples/demo_mirror/search_index/user_scope.yaml --output-format text
```
