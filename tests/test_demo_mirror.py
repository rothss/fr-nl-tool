...
from profiles.fr_example import profile_metadata  # noqa: E402
...
class DemoMirrorTests(unittest.TestCase):
    def test_profile_metadata_exposes_demo_queries(self) -> None:
        ...
        self.assertEqual(meta["profile_id"], "fr_example")
    ...
    def test_demo_mirror_supports_offline_runner_queries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            competition = runner.run_query(
                "海口-北京首都的指定航线，近三天的票价和客座率与外航相比，有没有什么异常或者可以改进的吗",
                mirror_root=root,
                db_path=catalog_db,
            ...
            self.assertIn("CompanyB", str(airline.get("answer_text") or ""))
