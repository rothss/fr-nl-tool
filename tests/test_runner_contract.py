...
class RunnerContractTests(unittest.TestCase):
    def test_runner_prefers_local_pipeline_before_legacy_executor(self) -> None:
        ...
        with (
            ...
            patch.object(
                runner,
                "execute_query",
                side_effect=AssertionError("legacy executor should not run"),
            ),
        ):
            result = runner.run_query(
                "海口到北京首都的客座率和票价，和竞争对手比有什么问题"
            )
        self.assertTrue(result["ok"])
        self.assertEqual(result["answer_text"], "本地分析答案")
        self.assertEqual(
            result["analysis_result"]["analysis_engine"], "future_flight_competition"
        )
    ...
    def test_runner_wraps_query_result_into_openclaw_envelope(self) -> None:
        ...
        with (
            ...
            patch.object(
                runner,
                "resolve_top_candidate",
                return_value=fake_result["top_candidate"],
            ),
            patch.object(
                runner,
                "build_initial_plan",
    ...
    def test_runner_maps_route_mismatch_to_structured_reason_when_not_refreshed(
        self,
    ) -> None:
        intent = {
            "metric": "客座率",
